...
const SHEET_NAME = "Ofícios";

const COLS = ["id","numero","dataRecebimento","origem","assunto","descricao","status","responsavel","observacoes"];
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      const values = [COLS, ...(oficios || []).map(o => COLS.map(c => o[c] ?? ""))];
      await sheets.spreadsheets.values.update({
        spreadsheetId: SPREADSHEET_ID,
        range: `${SHEET_NAME}!A1`,
        valueInputOption: "RAW",
